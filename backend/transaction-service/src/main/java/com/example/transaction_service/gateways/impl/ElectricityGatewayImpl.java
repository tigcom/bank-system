...
public class ElectricityGatewayImpl implements ProviderGateway {
    ...
    public BillDetailsResponse checkBill(String customerCode,String provider) {
        String fullUrl = this.apiUrl + "/check";
        log.info("[ELECTRICITY][checkBill] Gọi kiểm tra hóa đơn | customerCode: {} | provider: {} | endpoint: {}", customerCode, provider, fullUrl);
        ...
        try {
            ...
            if (apiResponse == null) {
                log.warn("[ELECTRICITY][checkBill] Không có dữ liệu trả về từ provider | customerCode: {} | provider: {}", customerCode, provider);
                throw new AppException(ErrorCode.BILL_NOT_FOUND);
            }
            log.info("[ELECTRICITY][checkBill] Kết quả trả về: {}", apiResponse);
            return apiResponse.getResult();
        } catch (HttpClientErrorException e) {
            log.error("[ELECTRICITY][checkBill] Lỗi client khi gọi API: {}", e.getResponseBodyAsString());
            if (e.getStatusCode() == HttpStatus.NOT_FOUND) {
                throw new AppException(ErrorCode.BILL_NOT_FOUND);
            }
            throw new AppException(ErrorCode.PROVIDER_SERVER_ERROR);
        } catch (Exception e) {
            log.error("[ELECTRICITY][checkBill] Lỗi server/provider: {}", e.getMessage());
            throw new AppException(ErrorCode.PROVIDER_SERVER_ERROR);
        }
    }
    ...
    public ProviderPaymentResponse payBill(ProviderPaymentRequest request) {
        ...
        log.info("[ELECTRICITY][payBill] Gọi xác nhận thanh toán | endpoint: {} | request: {}", fullUrl, request);
        ...
        try {
            ...
            if (apiResponse == null || apiResponse.getCode() != 200 || apiResponse.getResult() == null) {
                log.warn("[ELECTRICITY][payBill] Provider trả về lỗi | code: {} | message: {}", apiResponse != null ? apiResponse.getCode() : null, apiResponse != null ? apiResponse.getMessage() : null);
            ...
            log.info("[ELECTRICITY][payBill] Thanh toán thành công | providerTransactionId: {}", apiResponse.getResult().getProviderTransactionId());
            return apiResponse.getResult();
        } catch (HttpClientErrorException e) {
            log.error("[ELECTRICITY][payBill] Lỗi client khi gọi API: {}", e.getResponseBodyAsString());
            throw new AppException(ErrorCode.PROVIDER_PAYMENT_FAILED);
        } catch (RestClientException e) {
            log.error("[ELECTRICITY][payBill] Lỗi server/provider: {}", e.getMessage());
            throw new AppException(ErrorCode.PROVIDER_SERVER_ERROR);
        }
    }

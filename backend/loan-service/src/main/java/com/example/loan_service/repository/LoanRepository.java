...
public interface LoanRepository extends JpaRepository<Loan, Long> {
    ...
    @Query(value = "SELECT \n" +
            ...
            "  AND l.status ='approved' OR l.status ='CANCELLED';\n",nativeQuery = true)
    ...
    @Query(value = "select COALESCE(sum(l.amount )) from loan l where customer_id = :customerId and status in ( 'APPROVED','CLOSED','CANCELLED')",nativeQuery = true)

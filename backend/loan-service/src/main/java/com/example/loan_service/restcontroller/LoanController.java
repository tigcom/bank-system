...
public class LoanController {
    ...
    @PostMapping
    public ResponseEntity<ApiResponseWrapper<Loan>> createLoan(@Valid  @RequestBody LoanRequestDTO loan) {
        log.info("CREATE_LOAN_START - request: {}", loan);
        ApiResponseWrapper<Loan> response = new ApiResponseWrapper<>();
        try {

            Loan created = loanHandler.createLoan(loan.toCommonDto());
    ...
    @PutMapping
    public ResponseEntity<ApiResponseWrapper<Loan>> updateLoan(@Valid @RequestBody LoanRequestDTO loan) {
        log.info("UPDATE_LOAN_START - request: {}", loan);
        ApiResponseWrapper<Loan> response = new ApiResponseWrapper<>();
        try {
            Loan updated = loanHandler.updateLoan(loan.toCommonDto());
...
//    @PreAuthorize("hasRole('ADMIN')")
    ...
    @Operation(summary = "Lấy danh sách khoản vay đang chờ xử lý", description = "Truy vấn tất cả khoản vay có trạng thái PENDING")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Lấy danh sách khoản vay đang chờ xử lý thành công",
            content = @Content(mediaType = "application/json",
                schema = @Schema(implementation = ApiResponseWrapper.class),
                examples = @ExampleObject(value = "{\"status\":200,\"message\":\"Pending loans retrieved successfully\",\"data\":[{\"loanId\":123,\"amount\":10000,\"termMonths\":12,\"interestRate\":5.5,\"status\":\"PENDING\"}]}")
            )
        ),
        @ApiResponse(responseCode = "500", description = "Lỗi máy chủ khi lấy danh sách khoản vay đang chờ xử lý")
    })
    @GetMapping("/pending")
    public ResponseEntity<ApiResponseWrapper<List<Loan>>> getPendingLoans() {
        log.info("GET_PENDING_LOANS_START");
        ApiResponseWrapper<List<Loan>> response = new ApiResponseWrapper<>();
        try {
            List<Loan> pendingLoans = loanHandler.findPendingLoans();
            response.setData(pendingLoans);
            response.setStatus(HttpStatus.OK.value());
            response.setMessage("Pending loans retrieved successfully");
            log.info("GET_PENDING_LOANS_SUCCESS - count: {}", pendingLoans.size());
        } catch (Exception e) {
            log.error("GET_PENDING_LOANS_ERROR - error: {}", e.getMessage(), e);
            response.setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value());
            response.setMessage("Failed to retrieve pending loans: " + e.getMessage());
        }
        return new ResponseEntity<>(response, HttpStatus.valueOf(response.getStatus()));
    }
...
//    }
    ...
    @Operation(summary = "Lấy tổng số tiền đã thu hồi (loan CANCELLED)", description = "Chỉ dành cho ADMIN")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Lấy tổng số tiền đã thu hồi thành công",
            content = @Content(mediaType = "application/json",
                schema = @Schema(implementation = ApiResponseWrapper.class),
                examples = @ExampleObject(value = "{\"status\":200,\"message\":\"Total recovered amount (system-wide) retrieved\",\"data\":123456789}")
            )
        ),
        @ApiResponse(responseCode = "500", description = "Lỗi khi lấy tổng số tiền đã thu hồi")
    })
    @PreAuthorize("hasRole('ADMIN')")
    @GetMapping("/admin/total-recovered")
    public ResponseEntity<ApiResponseWrapper<BigDecimal>> getTotalRecoveredSystem() {
        log.info("GET_TOTAL_RECOVERED_SYSTEM_START");
        ApiResponseWrapper<BigDecimal> response = new ApiResponseWrapper<>();
        try {
            BigDecimal total = loanHandler.getTotalRecoveredSystem();
            response.setData(total);
            response.setStatus(HttpStatus.OK.value());
            response.setMessage("Total recovered amount (system-wide) retrieved");
            log.info("GET_TOTAL_RECOVERED_SYSTEM_SUCCESS - total: {}", total);
        } catch (Exception e) {
            log.error("GET_TOTAL_RECOVERED_SYSTEM_ERROR - error: {}", e.getMessage(), e);
            response.setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value());
            response.setMessage("Failed to get total recovered: " + e.getMessage());
        }
        return new ResponseEntity<>(response, HttpStatus.valueOf(response.getStatus()));
    }

...
public class LoanServiceImpl implements LoanService {
    ...
    public List<Loan> getLoansByCustomerId(Long customerId) {
        ...
        try {
            List<Loan> list = loanRepository.findAllByCustomerId(customerId);
    ...
    public BigDecimal getTotalOutstanding(Long customerId) {
        log.info("GET_TOTAL_OUTSTANDING_BY_CUSTOMER_START - customerId: {}", customerId);
        try {
            BigDecimal total = loanRepository.getTotalOutstanding(customerId);
            log.info("GET_TOTAL_OUTSTANDING_BY_CUSTOMER_SUCCESS - customerId: {}, total: {}", customerId, total);
            return total;
        } catch (Exception e) {
            log.error("GET_TOTAL_OUTSTANDING_BY_CUSTOMER_ERROR - customerId: {}, error: {}", customerId, e.getMessage(), e);
            throw e;
        }
    }

    @Override
    public BigDecimal getTotalOutstandingByLoan(Long loanId) {
        log.info("GET_TOTAL_OUTSTANDING_BY_LOAN_START - loanId: {}", loanId);
        try {
            BigDecimal total = loanRepository.getTotalOutstandingByLoan(loanId);
            log.info("GET_TOTAL_OUTSTANDING_BY_LOAN_SUCCESS - loanId: {}, total: {}", loanId, total);
            return total;
        } catch (Exception e) {
            log.error("GET_TOTAL_OUTSTANDING_BY_LOAN_ERROR - loanId: {}, error: {}", loanId, e.getMessage(), e);
            throw e;
        }
    }
    ...
    @Override
    @CacheEvict(value = {"loanById", "allLoans"}, allEntries = true)
    public Loan cancelledLoan(Long loanId) {
        log.info("CANCELL_LOAN_START - loanId: {}", loanId);

        try {
            Loan loan = loanRepository.findById(loanId)
                    .orElseThrow(() -> new EntityNotFoundException("Loan not found: " + loanId));
            if (!LoanStatus.APPROVED.equals(loan.getStatus())) {
                log.warn("CANCELL_LOAN_INVALID - loanId: {}, status: {}", loanId, loan.getStatus());
                throw new IllegalStateException("Loan is not in APPROVED status");
            }
            ...
            log.info("CANCELL_LOAN_SUCCESS - loanId: {}", saved.getLoanId());
            return saved;
        } catch (IllegalStateException e) {
            log.warn("CANCELL_LOAN_WARN - loanId: {}, reason: {}", loanId, e.getMessage());
            throw e;
        } catch (Exception e) {
            log.error("CANCELL_LOAN_ERROR - loanId: {}, error: {}", loanId, e.getMessage(), e);
            throw e;
        }
    }
    ...
    public BigDecimal getTotalDisbursedSystem() {
        ...
        try {
            BigDecimal total = loanRepository.sumAmountByStatuses(
                    List.of(LoanStatus.APPROVED, LoanStatus.CLOSED, LoanStatus.CANCELLED)
            );
    ...
    @Override
    public BigDecimal getTotalRecoveredSystem() {
        log.info("GET_TOTAL_RECOVERED_SYSTEM_START");
        try {
            BigDecimal total = loanRepository.sumAmountByStatuses(
                    List.of(LoanStatus.CANCELLED)
            );
    ...
    public List<Loan> getPendingLoans() {
        ...
        try {
            List<Loan> pendingLoans = loanRepository.findAllByStatusOrderByCreatedAtDesc(LoanStatus.PENDING);
            log.info("GET_PENDING_LOANS_SUCCESS - total: {}", pendingLoans.size());
            return pendingLoans;
        } catch (Exception e) {

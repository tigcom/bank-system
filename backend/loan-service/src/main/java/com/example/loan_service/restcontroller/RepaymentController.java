...
public class RepaymentController {
    ...
    public ResponseEntity<ApiResponseWrapper<String>> makeRepayment(
        ...
        try {
            ...
            String sub = e.getMessage().substring(e.getMessage().indexOf(":")+1,e.getMessage().indexOf("\n")+1);
            log.error("MAKE_REPAYMENT_ERROR - repaymentId: {}, error: {} +++++", repaymentId, sub);
            response.setData(null);
            response.setStatus(HttpStatus.INTERNAL_SERVER_ERROR.value());
            response.setMessage(sub);
        }
        return new ResponseEntity<>(response, HttpStatus.valueOf(response.getStatus()));
    }
    ...
    public ResponseEntity<ApiResponseWrapper<Repayment>> getCurrentRepayment() {
        ...
        try {
            ...
            if (current != null ) log.info("GET_CURRENT_REPAYMENT_HANDLER_SUCCESS - repaymentId: {}", current.getRepaymentId());

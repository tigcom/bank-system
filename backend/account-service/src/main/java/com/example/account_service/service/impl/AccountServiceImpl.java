...
public class AccountServiceImpl implements AccountService {
    ...
    public List<AccountPaymentResponse> getAllPaymentAccountsbyCifCode() {
        ...
        if (authentication == null || !authentication.isAuthenticated()) {
            log.warn("AUTHENTICATION_MISSING_OR_INVALID - Cannot get current user from SecurityContext");
            userId =RpcContext.getContext().getAttachment("username");
        }else {
    ...
    @Override
    public List<AccountPaymentResponse> getAllPaymentAccountsbyCifCode2() {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String userId =null;
        if (authentication == null || !authentication.isAuthenticated()) {
            log.warn("AUTHENTICATION_MISSING_OR_INVALID - Cannot get current user from SecurityContext");
        ...
        try {
            ...
            List<AccountType> types = Arrays.asList(AccountType.PAYMENT, AccountType.LOAN);
            List<Account> paymentAccounts = accountRepository.findByCifCodeAndAccountTypeInAndStatus(
                    cifCode, types, AccountStatus.ACTIVE);
            log.info("PAYMENT_ACCOUNTS_FOUND - UserId: {}, CifCode: {}, Count: {}",
                    userId, cifCode, paymentAccounts.size());
    ...
    public List<LoanAccountResponse> getAllLoanAccountsByCifCode() {
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String userId = authentication.getName();
        log.info("GET_ALL_LOAN_ACCOUNTS_START - UserId: {}", userId);
        try {
            CustomerDTO currentCustomer = commonService.getCurrentCustomer(userId);
            if (currentCustomer == null) {
                log.warn("GET_ALL_LOAN_ACCOUNTS_FAILED - UserId: {}, Reason: CUSTOMER_NOT_FOUND", userId);
                throw new AppException(ErrorCode.CUSTOMER_NOT_FOUND);
            }
            String cifCode = currentCustomer.getCifCode();
            log.info("CUSTOMER_FOUND - UserId: {}, CifCode: {}, CustomerStatus: {}", userId, cifCode, currentCustomer.getStatus());
            if (currentCustomer.getStatus() != CustomerStatus.ACTIVE) {
                log.warn("GET_ALL_LOAN_ACCOUNTS_FAILED - CifCode: {}, CustomerStatus: {}, Reason: CUSTOMER_NOT_ACTIVE",
                        cifCode, currentCustomer.getStatus());
                throw new AppException(ErrorCode.CUSTOMER_NOTACTIVE);
            }
            List<LoanAccount> loanAccounts = loanAccountRepository.findByCifCode(cifCode);
            if (loanAccounts == null || loanAccounts.isEmpty()) {
                log.warn("GET_ALL_LOAN_ACCOUNTS_EMPTY - CifCode: {}, Reason: NO_LOAN_ACCOUNT_FOUND", cifCode);
            } else {
                log.info("GET_ALL_LOAN_ACCOUNTS_SUCCESS - CifCode: {}, TotalAccounts: {}", cifCode, loanAccounts.size());
            }
            return loanAccounts.stream()
                    .map(acc -> {
                        BigDecimal balance = getBalanceFromCorebanking(acc.getAccountNumber());
                           return  LoanAccountResponse.builder()
                            ...
                            .loanId(acc.getLoanId())
                            .build();
                    }).collect(Collectors.toList());
        } catch (Exception e) {
            log.error("GET_ALL_LOAN_ACCOUNTS_ERROR - UserId: {}, Error: {}", userId, e.getMessage(), e);
            throw e;
        }
    }
    ...
    public AccountDTO createLoanAccount(LoanRequestDTO dto) {
        ...
        try {
            ...
            CustomerDTO admin = commonService.getCurrentCustomer(username);
            CustomerDTO customer = commonService.getCurrentCustomer2(dto.getCustomerId().toString());
            ...
            loanAccount.setCreatedBy(admin.getUsername());
    ...
    @Override
    public AccountCreateReponse createPaymentAccountForCustomer(String cifCode) {
    ...
    public void updateAccountFromLoan(LoanRequestDTO dto) {
        ...
        if (dto.getStatus().equals("CLOSED")) {
            ...
            log.info("[updateAccountFromLoan] getOutstandingDebt: {}", loanAccount.getOutstandingDebt());
            ...
            loanAccount.setOutstandingDebt(newOutstanding);

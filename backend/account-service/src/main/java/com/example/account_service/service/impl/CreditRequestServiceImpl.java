...
//        }
        log.info("[SCORING] Start decision | creditRequestId={} | cifCode={} | finalScore={}",
                creditRequestConfirmDTO.getCreditRequestId(),
                creditRequest.getCifCode(),
                finalScore);

        if (finalScore >= 80) {
            log.info("[SCORING] Path=PENDING_ADMIN | creditRequestId={} | finalScore={}",
                    creditRequestConfirmDTO.getCreditRequestId(), finalScore);

            creditRequest.setStatus(CreditRequestStatus.PENDING);
            log.debug("[SCORING] Updating status to PENDING | creditRequestId={} | beforeSaveStatus={}",
                    creditRequestConfirmDTO.getCreditRequestId(), creditRequest.getStatus());

            creditRequestRepository.save(creditRequest);
            log.info("[SCORING] Saved CreditRequest | creditRequestId={} | newStatus={}",
                    creditRequestConfirmDTO.getCreditRequestId(), creditRequest.getStatus());
            ...
            log.info("[SCORING] Deleted Redis keys for creditRequestId={}",
                    creditRequestConfirmDTO.getCreditRequestId());

            return mapToDto(creditRequest);

        } else {
            log.info("[SCORING] Path=AUTO_REJECT | creditRequestId={} | finalScore={}",
                    creditRequestConfirmDTO.getCreditRequestId(), finalScore);

            log.info("[SCORING] Calling autoRejectCreditRequest | creditRequestId={}",
                    creditRequestConfirmDTO.getCreditRequestId());
            return autoRejectCreditRequest(creditRequest, creditRequestConfirmDTO.getCreditRequestId());
        }
    ...
    public AccountCreateReponse approveCreditRequest(String id) {
        log.info("[APPROVE] Start approving credit request | requestId={}", id);
        CreditRequest creditRequest = getCreditRequestById(id);
        log.info("[APPROVE] Loaded CreditRequest | requestId={} | status={} | cifCode={} | accountNumber={}",
                creditRequest.getId(), creditRequest.getStatus(), creditRequest.getCifCode(), creditRequest.getAccountNumber());
        validateCreditRequestStatus(creditRequest);
        log.debug("[APPROVE] CreditRequest validated | requestId={} | status={}",
                creditRequest.getId(), creditRequest.getStatus());
        if (creditRequest.getStatus().equals(CreditRequestStatus.PENDING)) {
            log.info("[APPROVE] Path=PENDING → Creating new CreditAccount | requestId={}", creditRequest.getId());
            CreditAccount account = createCreditAccount(creditRequest);
            log.info("[APPROVE] Created CreditAccount | requestId={} | accountNumber={} | limit={}",
                    creditRequest.getId(), account.getAccountNumber(), account.getCreditLimit());
            creditRequest.setAccountNumber(account.getAccountNumber());
            Account account1 = accountRepository.findByAccountNumber(account.getAccountNumber());
            if (account1 == null) {
                log.error("[APPROVE] Account not found after create! | accountNumber={} | requestId={}",
                        account.getAccountNumber(), creditRequest.getId());
                throw new AppException(ErrorCode.ACCOUNT_NOT_FOUND);
            }
            log.debug("[APPROVE] Loaded Account entity | accountId={} | accountNumber={} | status={}",
                    account1.getId(), account1.getAccountNumber(), account1.getStatus());
            account1.setStatus(AccountStatus.ACTIVE);
            accountRepository.save(account1);
            log.info("[APPROVE] Updated Account to ACTIVE | accountId={} | accountNumber={}",
                    account1.getId(), account1.getAccountNumber());
            creditRequest.setStatus(CreditRequestStatus.APPROVED);
            creditRequestRepository.save(creditRequest);
            log.info("[APPROVE] Updated CreditRequest to APPROVED | requestId={} | accountNumber={}",
                    creditRequest.getId(), creditRequest.getAccountNumber());
            sendApprovalNotification(creditRequest, account1);
            log.info("[APPROVE] Sent approval notification | requestId={} | accountNumber={}",
                    creditRequest.getId(), account1.getAccountNumber());
            return buildAccountCreateResponse(account);
        } else {
            log.info("[APPROVE] Path=ALREADY_HAS_ACCOUNT → Reuse CreditAccount | requestId={} | accountNumber={}",
                    creditRequest.getId(), creditRequest.getAccountNumber());
            CreditAccount account = creditAccountRepository.findByAccountNumber(creditRequest.getAccountNumber());
            if (account != null) {
                log.info("[APPROVE] Found existing CreditAccount | accountNumber={} | limit={}",
                        account.getAccountNumber(), account.getCreditLimit());
                CardRegistrationMessage cardRegistrationMessage = CardRegistrationMessage.builder()
                        .accountNumber(account.getAccountNumber())
                        .cardType(account.getCreditCardType().getCardType())
                        .cifCode(account.getCifCode())
                        ...
                        .idRequest(creditRequest.getId())
                        .build();
                streamBridge.send("card-registration-topic", cardRegistrationMessage);
                log.info("[APPROVE] Sent CardRegistrationMessage to Kafka | topic=card-registration-topic | requestId={} | accountNumber={}",
                        creditRequest.getId(), account.getAccountNumber());

                return buildAccountCreateResponse(account);
            }
            log.error("[APPROVE] ACCOUNT_NOT_FOUND | requestId={} | expectedAccountNumber={}",
                    creditRequest.getId(), creditRequest.getAccountNumber());
            throw new AppException(ErrorCode.ACCOUNT_NOT_FOUND);
        }
    }
    ...
    private CreditAccount createCreditAccount(CreditRequest creditRequest) {
        log.info("[CREATE_ACCOUNT] Start | requestId={} | cifCode={} | cartTypeId={} | monthlyIncome={}",
                creditRequest.getId(), creditRequest.getCifCode(), creditRequest.getCartTypeId(), creditRequest.getMonthlyIncome());

        CreditCardType creditCardType = creditCardTypeRepository.findById(creditRequest.getCartTypeId())
                .orElseThrow(() -> {
                    log.error("[CREATE_ACCOUNT] CreditCardType not found | cartTypeId={} | requestId={}",
                            creditRequest.getCartTypeId(), creditRequest.getId());
                    return new AppException(ErrorCode.CORE_BANKING_SERVICE_ERROR);
                });
        log.info("[CREATE_ACCOUNT] Loaded CreditCardType | typeId={} | typeName={} | defaultLimit={}",
                creditCardType.getId(), creditCardType.getTypeName(), creditCardType.getDefaultCreditLimit());

        BigDecimal creditLimit = calculateCreditLimit(creditRequest.getMonthlyIncome(), creditCardType.getDefaultCreditLimit());
        log.info("[CREATE_ACCOUNT] Calculated creditLimit={} | requestId={}", creditLimit, creditRequest.getId());

//        CreditAccount existing = creditAccountRepository.findById(creditRequest.getId())
//                .orElseThrow(() -> {
//                    log.error("[CREATE_ACCOUNT] Existing CreditAccount not found | requestId={}", creditRequest.getId());
//                    return new AppException(ErrorCode.ACCOUNT_NOT_FOUND);
//                });
//        log.info("[CREATE_ACCOUNT] Found existing CreditAccount | accountId={} | status={}", existing.getId(), existing.getStatus());
//
//        existing.setStatus(AccountStatus.PENDING_EXTERNAL_REGISTER);
        ...
        log.debug("[CREATE_ACCOUNT] Prepared new CreditAccount entity | id={} | cifCode={} | status={} | limit={}",
                creditAccount.getId(), creditAccount.getCifCode(), creditAccount.getStatus(), creditAccount.getCreditLimit());

        String urlGetAccountNumber = coreBankingBaseUrl + "/getAccountNumber/{typeAccount}";
        log.info("[CREATE_ACCOUNT] Requesting account number from CoreBanking | url={} | typeAccount={}",
                urlGetAccountNumber, creditAccount.getAccountType().name());

        String number = coreBankingRestTemplate.getForObject(urlGetAccountNumber, String.class, creditAccount.getAccountType().name());
        log.info("[CREATE_ACCOUNT] Received accountNumber from CoreBanking | requestId={} | accountNumber={}",
                creditRequest.getId(), number);

        creditAccount.setAccountNumber(number);

        CreditAccount savedAccount = creditAccountRepository.save(creditAccount);
        log.info("[CREATE_ACCOUNT] Saved CreditAccount | accountId={} | accountNumber={} | cifCode={} | limit={} | status={}",
                savedAccount.getId(), savedAccount.getAccountNumber(), savedAccount.getCifCode(),
                savedAccount.getCreditLimit(), savedAccount.getStatus());
